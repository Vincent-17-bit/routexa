...
export async function rollupDay(day) {
  const targetDay = day || new Date(Date.now() + 3 * 3600000 - 86400000).toISOString().slice(0, 10)
  ...
  const logins = await db.execute({
    sql: `
      ...
      FROM login_logs
      WHERE deleted_at IS NULL AND date(timestamp, '+3 hours') = date(?)
      GROUP BY device_id
    `,
    args: [targetDay]
  })
  const searches = await db.execute({
    sql: `SELECT device_id, COUNT(*) AS n FROM search_logs WHERE deleted_at IS NULL AND date(timestamp, '+3 hours') = date(?) GROUP BY device_id`,
    args: [targetDay]
  })
  const routes = await db.execute({
    sql: `SELECT device_id, COUNT(*) AS n FROM route_logs WHERE deleted_at IS NULL AND date(timestamp, '+3 hours') = date(?) GROUP BY device_id`,
    args: [targetDay]
  })
